...
function generateBotResponse(message: string): string {
  ...
  if (lowerMessage.includes('adventure') || lowerMessage.includes('expedition') || lowerMessage.includes('retreat') || lowerMessage.includes('experience')) {
    return "Spectacular! Our adventure wellness escapes are absolutely incredible with luxury amenities and thrilling activities! Experience the magical Acadia Coastal Wellness Retreat with sunrise cliff yoga and sea cave kayaking ($6,800), the epic Katahdin Climbing Expedition with helicopter tours and luxury glamping ($8,500), the amazing Lakes Region adventure with treasure hunting and seaplane flights ($7,200), the spectacular Down East Coastal Explorer with private yacht charters and celebrity chef experiences ($9,400), or our ultimate Bali Soulshine Resort Adventure Escape with volcano helicopter tours, temple ceremonies, and luxury villa accommodations ($14,800)! Each adventure is packed with fun surprises, gourmet dining, and unforgettable memories! Which incredible adventure excites you most?";
  }
